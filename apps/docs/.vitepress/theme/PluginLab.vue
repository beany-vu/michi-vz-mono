...
function buildSeasonalSegments() {
  ...
  const cps = api.detectChangepoints(
    CP_VALS.map((p) => p[1]),
    {},
  );
  changepoints.value = cps.map((c: any) => ({
    year: CP_VALS[c.index][0],
...
function mountFeature() {
  ...
  try {
    if (feature === "sonify") {
      tones.value = api
        .valuesToTones(SONIFY_VALS, { duration: SONIFY_DUR })
        .map((t: any) => ({ freq: t.freq, value: t.value }));
      mountLine(
        host.value,
        [
          {
            label: "Daily visitors (k)",
            color: "#2563eb",
            series: SONIFY_VALS.map((v, i) => pt(2012 + i, v)),
          },
        ],
        { height: 220 },
      );
      ...
      const dataSet = aggRows.value.map((r) => ({
        label: String(r[groupKey.value]),
        valueBased: Number(r.revenue),
        valueCompared: Number(r.target),
      }));
      const margin = { top: 10, right: 26, bottom: 30, left: 120 };
      const h = margin.top + margin.bottom + Math.max(dataSet.length, 3) * 34;
      const c = api.mountComparableHorizontalBarChart(host.value, {
        dataSet,
        renderer: renderer.value,
      ...
      const histG =
        current / rampVals[0] > 0
          ? Math.pow(current / rampVals[0], 1 / (rampVals.length - 1)) - 1
          : 0;
      ...
      for (let k = 1; k <= horizon.value; k++)
        pace.push(pt(lastYear + k, current * Math.pow(1 + g, k), false));
      mountLine(
        host.value,
        [
      ...
      const res = api.monteCarloForecast(rampVals, {
        horizon: 4,
        runs: 400,
        seed: mcSeed.value,
        level: 0.9,
      ...
      mc.value = {
        runs: res.runs,
        pAbove: res.probabilityAbove(MC_TARGET),
        pBelow: res.probabilityBelow(MC_TARGET),
      };
      const lastYear = RAMP[RAMP.length - 1][0];
      // history as a thin band (min=max=value), forecast as a widening band of simulated futures.
      const hist = RAMP.map((p) => ({
        date: p[0],
        valueMin: p[1],
        valueMax: p[1],
        valueMedium: p[1],
      ...
      const fc = res.predictions.map((m: number, i: number) => ({
        date: lastYear + i + 1,
        valueMin: res.lower[i],
        valueMax: res.upper[i],
        valueMedium: m,
        certainty: true,
      }));
      const c = api.mountRangeChart(host.value, {
        dataSet: [{ label: "Revenue ($M)", color: "#2563eb", series: [...hist, ...fc] }],
      ...
      const sd = Math.sqrt(
        d.residual.reduce((a: number, b: number) => a + b * b, 0) / d.residual.length,
      );
      ...
      mountLine(
        ...
        [
          { label: "Observed", color: "#9aa4b2", series: SEAS_VALS.map((v, i) => pt(base + i, v)) },
          {
            label: "Trend",
            color: "#2563eb",
            series: d.trend.map((v: number, i: number) => pt(base + i, Math.round(v * 10) / 10)),
          },
        ],
        { height: 220 },
      );
      mountLine(
        ...
        [
          {
            label: "Seasonal (repeats)",
            color: "#16a34a",
            series: d.seasonal.map((v: number, i: number) => pt(base + i, Math.round(v * 10) / 10)),
          },
        ],
...
function reroll() {
  mcSeed.value = ((mcSeed.value * 1103515245 + 12345) % 2147483647) & 0xffff;
  mountFeature();
}
...
onMounted(async () => {
  try {
    ...
    api = {
      ...
      sonify: ins.sonify,
      valuesToTones: ins.valuesToTones,
      aggregate: ins.aggregate,
      requiredGrowth: ins.requiredGrowth,
      requiredRunRate: ins.requiredRunRate,
      monteCarloForecast: ins.monteCarloForecast,
      detectChangepoints: ins.detectChangepoints,
      detectPeriod: ins.detectPeriod,
      decompose: ins.decompose,
    };
    mountFeature();
    ro = new ResizeObserver(() => {
      cancelAnimationFrame(raf);
      raf = requestAnimationFrame(mountFeature);
    });
...
const TITLES: Record<string, string> = {
  sonify: "sonify · hear the trend",
  sql: "aggregate · rows → chart",
  goalseek: "goal-seek · what hits the target",
  montecarlo: "monteCarlo · a fan of futures",
  changepoints: "changepoints · where the trend bends",
  seasonal: "decompose · trend + season + noise",
};
    ...
    <div class="plab-content">
        ...
        <div class="plab-sonify">
          ...
          <div class="plab-tones" aria-hidden="true">
            <i
              v-for="(t, i) in tones"
              :key="i"
              :class="{ on: i === playingIdx }"
              :style="{ height: barH(t.freq) + 'px' }"
            ></i>
          </div>
        </div>
        <p class="plab-note">
          Each value becomes a pitch - low value, low note (220 Hz) up to a high note (880 Hz). A
          rising series sweeps up; a dip dips. <code>sonify()</code> is a graceful no-op without Web
          Audio, and <code>valuesToTones()</code> (the bars) is pure and testable. An accessibility
          win: the trend is now <em>audible</em>.
        </p>
      </template>
      ...
      <template v-else-if="feature === 'sql'">
        <div class="plab-sql">
          <div class="plab-rawtable">
            ...
            <table>
              ...
              <tbody>
                <tr v-for="(r, i) in SQL_ROWS.slice(0, 6)" :key="i">
                  <td>{{ r.region }}</td>
                  <td>{{ r.product }}</td>
                  <td>{{ r.revenue }}</td>
                  <td>{{ r.target }}</td>
        ...
        <p class="plab-note">
          ...
          >
          - rolls {{ SQL_ROWS.length }} rows into <strong>{{ aggRows.length }}</strong> bars
          (revenue vs target, sorted). Model-free and deterministic; opt into DuckDB-Wasm for real
          SQL over millions of rows.
        </p>
      </template>
      ...
      <template v-else-if="feature === 'goalseek'">
        ...
        <div v-if="goal" class="plab-verdict" :class="{ ok: goal.onTrack, bad: !goal.onTrack }">
          <div class="plab-verdict-line">
            From <strong>{{ goal.current }}</strong> to <strong>{{ target }}</strong> in
            {{ horizon }} periods needs
            <strong>{{ (goal.g * 100).toFixed(1) }}% / period</strong> (compounding) - or
            <strong>+{{ goal.rr.toFixed(1) }} / period</strong> flat.
          </div>
          <div class="plab-verdict-line">
            Recent pace is {{ (goal.histG * 100).toFixed(1) }}% / period →
            <strong>{{ goal.onTrack ? "on track ✓" : "behind, stretch needed" }}</strong
            >.
          </div>
        </div>
        <p class="plab-note">
          Forecasting runs time forward; <strong>goal-seek runs it backward</strong> from a target.
          <code>requiredGrowth()</code> and <code>requiredRunRate()</code> are plain arithmetic -
          the gold dashed line is the pace you'd have to hold. Change the target and watch it move.
        </p>
      </template>
      ...
      <template v-else-if="feature === 'montecarlo'">
        ...
        <div v-if="mc" class="plab-statrow">
          <div class="plab-stat">
            <span class="plab-statnum">{{ mc.runs }}</span
            ><span class="plab-statlbl">simulated futures</span>
          </div>
          <div class="plab-stat ok">
            <span class="plab-statnum">{{ pct(mc.pAbove) }}%</span
            ><span class="plab-statlbl">finish above {{ MC_TARGET }}</span>
          </div>
          <div class="plab-stat bad">
            <span class="plab-statnum">{{ pct(mc.pBelow) }}%</span
            ><span class="plab-statlbl">finish below {{ MC_TARGET }}</span>
          </div>
        </div>
        <p class="plab-note">
          A single forecast line hides the risk. <code>monteCarloForecast()</code> runs hundreds of
          futures - each step nudged by random shocks - and reports the <strong>band</strong> (the
          shaded range) plus the odds of clearing a target. Deterministic via a seeded RNG, so the
          same seed always replays; <strong>Re-roll</strong> shows how the spread shifts.
        </p>
      </template>
      ...
      <template v-else-if="feature === 'changepoints'">
        ...
        <div v-if="changepoints.length" class="plab-cps">
          <div v-for="(c, i) in changepoints" :key="i" class="plab-cp">
            Trend bent at <strong>{{ c.year }}</strong
            >: slope {{ c.slopeBefore.toFixed(1) }} → {{ c.slopeAfter.toFixed(1) }}
            <span class="plab-cp-tag" :class="{ down: c.slopeAfter < c.slopeBefore }">{{
              c.slopeAfter < c.slopeBefore ? "▼ turned down" : "▲ turned up"
            }}</span>
          </div>
        </div>
        <p class="plab-note">
          Averages hide the moment a story changes. <code>detectChangepoints()</code> finds where
          the <em>slope</em> structurally shifts and the line is coloured by regime - here a clear
          peak-then-decline. Useful for "when did growth stall?" without eyeballing.
        </p>
      </template>
      ...
      <template v-else-if="feature === 'seasonal'">
        ...
        <p v-if="seasonal" class="plab-note">
          <code>decompose()</code> splits the wiggly <strong>observed</strong> line into a smooth
          <strong>trend</strong> (top) and a repeating <strong>seasonal</strong> wave (bottom).
          <code>detectPeriod()</code> found the cycle length on its own:
          <strong>{{ seasonal.period }}</strong> steps. What's left over - the unexplained noise -
          has a standard deviation of just <strong>{{ seasonal.residSd }}</strong
          >, so trend + season explain almost all of it. This is how you separate "we grew" from
          "it's December again."
        </p>
      </template>
...
.plab-play.playing {
  background: var(--mv-gold, #cda14a);
  border-color: var(--mv-gold, #cda14a);
}
.plab-tones {
  display: flex;
  align-items: flex-end;
...
.plab-tones i {
  flex: 1;
  min-width: 4px;
  background: linear-gradient(var(--vp-c-brand-1), var(--vp-c-brand-2, #b23a2e));
...
.plab-tones i.on {
  ...
  box-shadow: 0 0 8px -2px var(--mv-gold-bright, #e7b143);
}
...
.plab-rawtable td:first-child,
.plab-rawtable th:first-child,
.plab-rawtable td:nth-child(2),
.plab-rawtable th:nth-child(2) {
  text-align: left;
}
.plab-rawmore {
  font-size: 11px;
...
.plab-verdict.bad {
  ...
  border-color: var(--mv-gold-bright, #e7b143);
}
.plab-verdict-line {
  color: var(--vp-c-text-1);
}
...
.plab-stat.bad {
  border-color: rgba(231, 177, 67, 0.5);
}
.plab-statnum {
  display: block;
  font-family: "Josefin Sans", system-ui, sans-serif;
...
.plab-stat.ok .plab-statnum {
  color: #16a34a;
}
.plab-stat.bad .plab-statnum {
  color: #b8860b;
}
.plab-statlbl {
  display: block;
...
@keyframes plab-glow {
  ...
  100% {
    box-shadow:
      0 0 0 1px rgba(147, 31, 26, 0.14),
      0 0 14px -5px rgba(147, 31, 26, 0.36);
  }
  50% {
    box-shadow:
      0 0 0 1px rgba(231, 177, 67, 0.3),
      0 0 22px -3px rgba(231, 177, 67, 0.46);
  }
}
.plab.ai-glow {
  animation: plab-glow 4.6s ease-in-out infinite;
}
@media (prefers-reduced-motion: reduce) {
  .plab.ai-glow {
    animation: none;
    box-shadow: 0 0 14px -5px rgba(147, 31, 26, 0.36);
  }
  .plab-tones i {
    transition: none;
  }
