...
const PREVIEW_GROUPS = [
  { name: "United States", total: 80, members: ["United States", "United  States", "united states", "USA"] },
...
const { status: llmStatus, pct: llmPct, errMsg: llmErr, load: llmLoad, generate: llmGen } = useLlm();
...
const shown = computed(() => (mode.value === "certified" && certified.value.length ? certified.value : groups.value));
...
function representative(members: string[], vecs: number[][], cos: (a: number[], b: number[]) => number): string {
  let bi = 0;
  let bs = -Infinity;
...
async function reconcile() {
  ...
  const vecs = await embed(RAW.map((r) => r.label), backend.value);
  ...
  RAW.forEach((row, i) => {
    ...
    for (const c of cl) {
      let s = 0;
      for (const v of c.vecs) { const x = cos(vecs[i], v); if (x > s) s = x; } // single-linkage
      if (s > bestSim) { secondSim = bestSim; bestSim = s; best = c; }
  ...
  groups.value = cl
    .map((c) => ({ name: representative(c.members, c.vecs, cos), total: c.total, members: c.members }))
...
function cleanCountry(s: string): string {
  let t = (s.split("\n").find((l) => l.trim()) ?? "").trim();
  t = t.replace(/^(the\s+country\s+is\s+|country\s*[:=]?\s*|answer\s*[:=]?\s*|it'?s\s+|that'?s\s+)/i, "");
  t = t.replace(/^["'`*\s]+|["'`*.\s]+$/g, "");
  return t || s.trim();
}
...
const ALIAS: Record<string, string> = {
  ...
  nederland: "Netherlands", holland: "Netherlands", espana: "Spain", italia: "Italy", brasil: "Brazil",
};
function canonicalize(name: string): string {
  const key = name
    .normalize("NFD").replace(/[̀-ͯ]/g, "")
    .toLowerCase().replace(/[^a-z0-9\s]/g, "")
    .replace(/\s+/g, " ").replace(/^the\s+/, "").trim();
...
async function certify() {
  ...
  try {
    ...
    for (const g of groups.value) {
      ...
      const key = canon.toLowerCase().replace(/^the\s+/, "").replace(/[^a-z0-9]/g, ""); // group-safe key
      const cur = byCanon.get(key);
      if (cur) { cur.total += g.total; cur.members.push(...g.members); }
...
function render() {
  ...
  const ds = mode.value === "raw"
    ...
    : shown.value.map((g, i) => ({ label: g.name, valueBased: g.total, valueCompared: 0, color: PALETTE[i % PALETTE.length] }));
...
<template>
  <div class="elab">
    <div class="elab-bar">
      <ol class="elab-steps">
        ...
        <li :class="{ on: mode === 'reconciled', done: mode === 'certified' }">
          <button class="elab-step" @click="setMode('reconciled')"><span class="n">{{ mode === 'certified' ? '✓' : '2' }}</span> Reconcile</button>
        </li>
        <li :class="{ on: mode === 'certified' }">
          <button class="elab-step" @click="setMode('certified')"><span class="n">3</span> Certify</button>
        </li>
      </ol>
      <div class="elab-ctrls">
        <div class="elab-modes" role="group" aria-label="Result mode">
          ...
          <button :class="{ on: !preview }" @click="setPreview(false)"
            title="Real model: download the actual model (MiniLM ~23 MB to reconcile; a small LLM for certify) and run it live in your browser - WebGPU, nothing sent to a server.">Real model</button>
        </div>
    ...
    <p class="elab-scenario">
      ...
      <strong>10 different ways</strong>. <strong>Reconcile</strong> merges by similarity (the embedding model);
      <strong>Certify</strong> adds a second specialist - a small LLM that confirms each merge and names it.
    ...
    <p class="elab-result">
      ...
      <template v-if="mode === 'certified'">→ <strong class="elab-hit">{{ shown.length }}</strong>
    ...
    <div v-if="mode === 'certified' && !preview" class="elab-certify">
      <div class="elab-llm">
        ...
        <button class="elab-go" @click="certify" :disabled="certifying">
          <span v-if="certifying && llmStatus === 'loading'">Loading {{ selectedLlm.name }}… {{ llmPct }}%</span>
          <span v-else-if="certifying">Certifying…</span>
          <span v-else>Run ✦</span>
        </button>
      </div>
      <p class="elab-note">
        A <strong>cascade</strong>, not one big model: the embedding model proposes the merges, this small LLM
        names each group, and a tiny alias list tidies rote abbreviations (<code>USA</code> → United States) so
        synonyms fold in reliably. The weights download <strong>once</strong> ({{ fmtSize(selectedLlm.sizeMB) }})
        and run in your browser (WebGPU) - nothing is sent to a server.
      </p>
      <p v-if="llmStatus === 'error'" class="elab-err">⚠ {{ llmErr }} (a recent Chrome/Edge with WebGPU is needed for in-browser LLMs).</p>
    </div>
    ...
    <p class="elab-legend">
      <template v-if="mode === 'certified'">
        <span v-if="preview">A small LLM confirms each group is one country and stamps the authoritative name -
          <code>USA</code> → United States, <code>Deutschland</code> → Germany, <code>Nippon</code> → Japan, with correct totals.
          <strong>Shown instantly</strong>; switch to <strong>Real model</strong> to download a small LLM ({{ selectedLlm.name }}) and run it.</span>
        <span v-else>{{ selectedLlm.name }} read each group's <em>meaning</em>, confirmed they're the same country, and returned
          the authoritative name - so even <code>USA</code>, <code>Deutschland</code> and <code>Nippon</code> fold in with correct totals.</span>
      </template>
      <template v-else-if="mode === 'reconciled'">
        <span v-if="preview">An embedding model groups the 10 spellings <em>by meaning</em> into the 3 real countries -
          even <code>USA</code> / <code>Deutschland</code> / <code>Nippon</code>, which share no letters.
          <strong>Shown instantly</strong>; switch to <strong>Real model</strong> to download MiniLM (~23 MB) and run it.</span>
        <span v-else-if="backend === 'hash'">Model-free merges <em>spelling</em> (case, spaces, typos) offline - but
          <code>USA</code>, <code>Deutschland</code>, <code>Nippon</code> stay split (no shared letters).
          <strong>Load {{ embName }}</strong> (top-right) to merge by meaning.</span>
        <span v-else><strong>{{ embName }}</strong> merged the abbreviations and translations too - down to the 3 real countries.
          Use <strong>Certify</strong> to have a small LLM confirm each and stamp the canonical name.</span>
      </template>
      <template v-else>Charted raw, each spelling is its own bar - the totals are wrong and split. Step through
