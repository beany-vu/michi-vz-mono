...
Gắn `date` cho từng liên kết rồi bật `timeline`: snapshot của một năm là các liên kết chia sẻ `date` đó - các nút dùng chung qua mọi năm, chỉ có dòng chảy đến rồi đi - và độ dày dải tự biến hình mượt giữa các năm. Mặc định tắt - không bật thì biểu đồ giữ nguyên. Đây là kiểu chạy từng năm có tương tác, không phải hiệu ứng vào cảnh một lần bên dưới.

<TimelinePlayDemo chart="sankey-chart" hint="Bấm nút play dưới biểu đồ: dữ liệu chạy qua từng năm, mỗi lần một snapshot. Kéo thanh tua để nhảy đến năm bất kỳ." />
...
- Liên kết không có `date` vẫn hiển thị ở mọi giai đoạn.
...
<RevealDemo chart="sankey-chart" replay-label="Chạy lại hiệu ứng" hint="Mỗi đường lớn dần từ năm đầu đến năm cuối; nhãn bám theo ngọn đường rồi dừng ở điểm cuối. Khi hệ điều hành bật reduced motion, biểu đồ hiển thị đầy đủ ngay lập tức." />
