...
describe("BarBellChart progressiveDraw off by default", () => {
...
describe("BarBellChart progressiveDraw SVG reveal", () => {
  it("clips the marks group only (not the axes/title)", () => {
    const ticker = createManualTicker();
    const { host, chart } = mount({ progressiveDraw: true }, ticker);
    const root = host.querySelector("g.bar-bell-content")!;
    ...
    const axis = host.querySelector(".mv-x-axis");
    expect(axis?.getAttribute("clip-path") ?? null).toBeNull();
    chart.destroy();
    host.remove();
  });
...
describe("BarBellChart progressiveDraw canvas mode", () => {
