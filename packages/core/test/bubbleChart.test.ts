...
describe("mountBubbleChart (jsdom)", () => {
  ...
  it("carries legendData with the veiled paleColor companion when a split is active", () => {
    const withSplit = mount({ dataSet: data, colorsMapping: { Germany: "#c0392b" } });
    const ctx = withSplit.chart.getContext()!;
    if (ctx.chartType === "bubble-chart") {
      const germany = ctx.legendData!.find((l) => l.label === "Germany")!;
      expect(germany.color).toBe("#c0392b");
      // splitOpacity default 0.35 -> veil 0.65 -> the exact white-mix the renderer paints.
      expect(germany.paleColor).toBe("#e9bab5");
    }
    withSplit.chart.destroy();
    withSplit.host.remove();

    const noSplit = mount({ dataSet: data, showSplit: false });
    const ctx2 = noSplit.chart.getContext()!;
    if (ctx2.chartType === "bubble-chart") {
