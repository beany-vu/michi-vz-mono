...
describe("FountainChart progressiveDraw off by default", () => {
  it("renders no clipPath and full jets when the prop is unset", () => {
...
describe("FountainChart progressiveDraw SVG reveal", () => {
  it("clips the marks group only (not the title)", () => {
    const ticker = createManualTicker();
    const { host, chart } = mount({ progressiveDraw: true }, ticker);
    const root = host.querySelector("g.fountain-chart-content")!;
