...
// RadialTree's "webgpu" renderer DELEGATES to the canvas-2D renderer (see
// radialTree/renderWebgpu.ts's header comment - same rationale as ChoroplethMap's/
// SymbolMap's: the dendrogram's links are curved cubic beziers, so tessellating
// them correctly on the GPU is disproportionate scope here). No async device, no
// canvas-2D "stopgap while loading" phase - it always paints synchronously and
// always returns true. These tests verify the SAME capability-gate + host-wiring
// contract every other chart's webgpu test file checks.
...
describe("mountRadialTreeChart - webgpu renderer (capability gate + delegated canvas paint)", () => {
  ...
  it("does not render SVG circle marks in webgpu mode (delegated to canvas)", () => {
    setGpu(true);
    const { host, chart } = mount({ renderer: "webgpu" });
  ...
  it("reuses the canvas-mode host hit-test: a mousemove fires onHighlightItem", () => {
    ...
    host.dispatchEvent(new MouseEvent("mousemove", { clientX: 300, clientY: 200, bubbles: true }));
