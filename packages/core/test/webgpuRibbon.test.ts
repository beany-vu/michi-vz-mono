...
function mount(extra: Partial<RibbonChartProps> = {}) {
  ...
  const chart = mountRibbonChart(host, { series, keys, width: 600, height: 300, ...extra });
  return { host, chart };
}
...
describe("mountRibbonChart - webgpu renderer (capability gate + fallback)", () => {
  ...
  it("does not render SVG rect.bar / path.ribbon marks in webgpu mode (painted to the GPU/canvas layer)", () => {
    setGpu(true);
    const { host, chart } = mount({ renderer: "webgpu" });
    expect(host.querySelectorAll("rect.bar").length).toBe(0);
    expect(host.querySelectorAll("path.ribbon").length).toBe(0);
    chart.destroy();
    host.remove();
  });
  ...
  it("reuses the canvas-mode host hit-test: a mousemove over a column fires onHighlightItem", () => {
