...
describe("mountComparableHorizontalBarChart — webgpu renderer (capability gate + fallback)", () => {
  ...
  it("reuses the canvas-mode host hit-test: a mousemove over a bar fires onHighlightItem", () => {
    ...
    // the model's pixel coordinates. width=600,height=300, default margins
    // (top/right/bottom=50, left=120) put the first band roughly mid-left.
    host.dispatchEvent(new MouseEvent("mousemove", { clientX: 300, clientY: 65, bubbles: true }));
