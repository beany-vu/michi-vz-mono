...
describe("mountLineChart mouse-line stacking vs the marks canvas layer", () => {
  // The marks canvas is a LATER host sibling than the base svg, so with
  // z-index:auto it paints ABOVE svg-internal chrome. Painted renderers
  // therefore re-home the mouse line into a dedicated overlay svg inserted
  // AFTER the canvas (before the tooltip); svg mode keeps the legacy in-svg
  // placement.
  ...
  it("canvas renderer: the mouse line's overlay svg is a LATER sibling than the marks canvas", () => {
    const { host, chart } = mount({ renderer: "canvas" });
    const canvas = host.querySelector("canvas.line-chart-canvas")!;
    const line = host.querySelector("line.mv-mouse-line")!;
  ...
  it("svg renderer (default): the mouse line stays inside the single main svg - unchanged", () => {
    const { host, chart } = mount();
    expect(host.querySelectorAll("svg").length).toBe(1);
    expect(host.querySelector("svg")!.contains(host.querySelector("line.mv-mouse-line")!)).toBe(
      true,
    );
    chart.destroy();
    host.remove();
  ...
  it("canvas mode: update() keeps exactly one mouse line and one overlay svg, still above the canvas", () => {
    const { host, chart } = mount({ renderer: "canvas" });
    chart.update({ ...base, renderer: "canvas" });
    expect(host.querySelectorAll("line.mv-mouse-line").length).toBe(1);
  ...
  it("nodata frame drops the overlay; the data round-trip rebuilds it above the recreated canvas", () => {
    const { host, chart } = mount({ renderer: "canvas" });
    chart.update({ ...base, dataSet: [], renderer: "canvas" }); // skipScaffold: no line, no canvas
    expect(host.querySelector("line.mv-mouse-line")).toBeNull();
    expect(host.querySelectorAll("svg").length).toBe(1);
    chart.update({ ...base, renderer: "canvas" });
    const canvas = host.querySelector("canvas.line-chart-canvas")!;
