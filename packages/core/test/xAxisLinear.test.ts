...
describe("renderXAxisLinear adaptive density (autoRotate + maxTicks)", () => {
  ...
  it("keeps ALL labels and tilts them -45° at medium density (fits rotated, not horizontal)", () => {
    ...
    const labels = render(
      scaleLinear().domain([0, 7]).range([20, 230]),
      { autoRotate: true },
      8,
    );
    ...
    expect(
      labels.every((l) =>
        (l.getAttribute("transform") ?? "").includes("rotate(-45)"),
      ),
    ).toBe(true);
  });
  ...
  it("keeps ALL labels horizontal when they comfortably fit", () => {
    const labels = render(
      scaleLinear().domain([0, 1]).range([20, 780]),
      { autoRotate: true },
      2,
    );
    expect(labels.length).toBe(2);
    expect(
      labels.every((l) => l.getAttribute("text-anchor") === "middle"),
    ).toBe(true);
    expect(
      labels.every(
        (l) => !(l.getAttribute("transform") ?? "").includes("rotate"),
      ),
    ).toBe(true);
  });
  ...
  it("honours maxTicks as a hard cap when autoRotate is off (no rotation)", () => {
    const labels = render(
      scaleLinear().domain([0, 11]).range([20, 780]),
      { maxTicks: 4 },
      12,
    );
    const texts = labels.map((l) => l.textContent);
...
describe("renderXAxisLinear no-data ticks (fillPeriodTicks marking)", () => {
  ...
  it("marks ONLY the noDataValues labels faded + tags them with data-mv-value", () => {
    ...
    expect(faded.map((l) => l.getAttribute("data-mv-value")).sort()).toEqual([
      "1",
      "3",
    ]);
    // faded labels keep the base class AND become hover targets
    expect(faded.every((l) => cls(l).includes("mv-axis-label"))).toBe(true);
    expect(faded.every((l) => l.getAttribute("pointer-events") === "all")).toBe(
      true,
    );
    // the other three labels are untouched
    expect(
      labels.filter((l) => !cls(l).includes("mv-tick-nodata")).length,
    ).toBe(3);
  });
