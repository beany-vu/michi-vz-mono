...
describe("VerticalStackBarChart timeline off by default", () => {
...
describe("VerticalStackBarChart timeline (cumulative)", () => {
  ...
  it("stepForward() sweeps the bars to the next period", () => {
    const ticker = createManualTicker();
...
describe("VerticalStackBarChart timeline + no-data state", () => {
  it("skips the timeline gracefully when the chart is in the no-data state", () => {
    const ticker = createManualTicker();
    const { host, chart } = mount({ timeline: true, isNodata: true }, ticker);
    expect(host.querySelector("clipPath")).toBeNull();
    expect(host.querySelector(".mv-timeline")).toBeNull();
...
describe("VerticalStackBarChart timeline canvas mode", () => {
...
describe("VerticalStackBarChart timeline webgpu mode", () => {
  afterEach(() => {
    setGpu(false);
