...
function mount(extra: Partial<DualBarChartProps> = {}, ticker?: ManualTicker) {
...
describe("dual horizontal bar chart timeline (enabled)", () => {
  ...
  it("keeps the user's own filter while playing (top-1 by value1 per period)", () => {
    const { host, chart } = mount({
      timeline: true,
      filter: { limit: 1, criteria: "value1", sortingDir: "desc" },
    });
...
describe("dual horizontal bar chart timeline control UI", () => {
