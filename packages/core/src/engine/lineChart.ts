...
export function mountLineChart(
  ...
  let props: LineChartProps = initial;
  ...
  const showTooltip = (label: string, ev: MouseEvent): void => {
    ...
    const item = props.dataSet.find((s) => s.label === label);
    const htmlStr = props.tooltipFormatter
      ? props.tooltipFormatter(hit.d, hit.series, props.dataSet)
  ...
  const onHostMove = (ev: MouseEvent): void => {
    ...
    if (hitLabel) {
      ...
      props.onHighlightItem?.([hitLabel]);
    } else {
      hideTooltip();
  ...
  function render(): void {
    ...
    if (props.onDataWarning) {
      const warnings = checkLineData(props.dataSet, xAxisDataType);
