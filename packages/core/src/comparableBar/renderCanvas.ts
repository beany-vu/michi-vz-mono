// Opt-in Canvas 2D renderer for ComparableHorizontalBar. Each label has two
// sub-bars (value-based behind, value-compared in front). Sub-bar fill colours are
// resolved via DUAL nested probes so descendant consumer CSS
// (`.bar[data-label-safe] .value-based`) matches; ['fill','stroke'] lets a
// `url(#pattern)` fill fall through to the stroke colour. value-based may instead
// be filled with a hatch PATTERN from patternsMapping (the data-URI from
