...
const fr: NarrateStrings = {
  topMover: (label, dir, pct) =>
    `${label} a ${dir === "rose" ? "le plus augmenté" : dir === "fell" ? "le plus baissé" : "stagné"}${pct}.`,
...
describe("narrate customization + i18n", () => {
  ...
  it("narrate({ render }) fully replaces the narration", () => {
    const out = narrate({ render: (c) => `[custom] ${c.chartType}` }).enrichContext!(
      ctx,
      {} as never,
    ) as { summary: string };
  ...
  it("explainChart rules fallback honours strings/render", async () => {
    expect(await explainChart(ctx, { backend: "rules", strings: fr })).toContain(
      "le plus augmenté",
    );
    expect(await explainChart(ctx, { backend: "transformers", render: () => "RENDERED" })).toBe(
      "RENDERED",
    ); // dep absent → fallback to render
  });
});
